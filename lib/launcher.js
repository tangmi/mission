...
function startApp(service) {
	...
	if(service.static) {
		service.static = path.join(appDir, service.static);
	}
	...
	if (service.start) {
		...
		process.env.PORT = service.attr('port');
		...
		//TODO: Store pidfile in mission folder/pid

		logger.info('%s: started (port=%d, pid=%d)', service.name, port, processes[service.name].pid);
	} else {
		//no launch script? no problem.
		service.attr('static-only', true);
		logger.info('%s: static-only site', service.name);
	}
