...
(function(exports) {
	...
	function add(serviceName, hostname) {
		...
		configure.set('sites', sites, function() {
			reloadConfigs();
		});
	}
	...
	function enable(serviceName) {
		...
		if (sites[serviceName]) {
			...
			configure.set('sites', sites, function() {
				configure.queue.add(serviceName, function() {
					reloadConfigs();
				});
			});
		} else {
	...
	function list() {
		...
		for (var serviceName in sites) {
			if (sites.hasOwnProperty(serviceName)) {
				...
				available.push(serviceName + ': ' + site.hostname + ' (local port: ' + site.port + ')');
				if (site.enabled) {
	...
	function reloadConfigs() {
		...
		var dirNginx = configure.get('paths.sites-enabled');
		if (!dirNginx || !fs.existsSync(dirNginx)) {
			console.log('"' + dirNginx + '"" is not a real directory!');
			return;
		}
		dirNginx = path.resolve(dirNginx);
		...
		for (var serviceName in sites) {
			if (sites.hasOwnProperty(serviceName)) {
				...
				if (site.enabled) {
					...
					fs.writeFileSync(path.join(dirNginx, serviceName), nginxConfig, 'utf8', function() {
						//we need this callback for some reason to quench an error from fs
					});
				}
			}
		}
